...
class MaskDecoderSSQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.compress_vit_feat = subnet(vit_dim,1, 4)
        
        self.embedding_encoder = subnet(transformer_dim,1, 4)
    ...
    def forward(
        ...
        hq_features = self.embedding_encoder(image_embeddings) + self.compress_vit_feat(vit_features)  ## 早期加后期特征 
    ...
    def predict_masks(
        ...
        ## self.embedding_maskfeature(upscaled_embedding_sam) 是maskdecoder feature
...
def show_mask(mask, ax, random_color=False):
    if random_color:
        ...
        # color = np.array([30/255, 144/255, 255/255, 0.5])  # 半透明绿色,test1
        # color = np.array([255/255, 255/255, 102/255, 0.6])  # 半透明淡黄色，ours
        # color = np.array([255/255, 165/255, 0, 0.6])
        ### 其他
        # color = np.array([128/255, 0/255, 128/255, 0.6])
        color = np.array([255/255, 182/255, 193/255, 0.7])  # 半透明粉色
...
def get_args_parser():
    parser = argparse.ArgumentParser('Plant-SAM', add_help=False)

    parser.add_argument("--output", type=str, default='./out',required=False, 
                        help="Path to the directory where masks and checkpoints will be output")
    parser.add_argument("--model-type", type=str, default="vit_h", 
                        help="The type of model to load, in ['vit_h', 'vit_l', 'vit_b']")
    parser.add_argument("--checkpoint", type=str, default='./pretrained_checkpoint/sam_vit_h_4b8939.pth',required=False, 
...
def main(net, train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        if args.resume:
            ...
            if torch.cuda.is_available():
                net_without_ddp.load_state_dict(torch.load(args.resume),strict = False)
            else:
                net_without_ddp.load_state_dict(torch.load(args.resume,map_location="cpu"),strict = False)
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    if misc.is_main_process():
        ...
        model_name = "/plant_sam_epoch_"+str(epoch)+".pth"
...
def evaluate(args, net, sam, valid_dataloaders, visualize=False,write_txt=True):
    net.eval()
    print("Validating...")
    test_stats = {}
...
if __name__ == "__main__":
    ...
    dataset_train = {"name": "LeafSeg-22K",
                 "im_dir": "./LeafSeg-22K/train",
                 "gt_dir": "./LeafSeg-22K/train",
                 "im_ext": ".jpg",
                 "gt_ext": ".png"}


    dataset_val = {"name": "LeafSeg-22K",
                "im_dir": "./LeafSeg-22K/val",
                "gt_dir": "./LeafSeg-22K/val",
                "im_ext": ".jpg",
                "gt_ext": ".png"}
    ...
    net = MaskDecoderSSQ(args.model_type)
